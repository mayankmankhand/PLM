...
- **Security**: Rate limiting on `/api/chat` (10 req/min per IP via in-memory sliding window in `src/lib/rate-limit.ts`), security headers in `next.config.ts` (X-Content-Type-Options, X-Frame-Options, Referrer-Policy, Permissions-Policy), `skipHtml` on ReactMarkdown, trace logging guarded behind `NODE_ENV !== 'production'`, UUID format validation on `x-demo-user-id` header in Edge Middleware
