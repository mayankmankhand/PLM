...
export function TableView({ payload, onNavigate }: TableViewProps) {
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm rounded-xl border border-border-subtle shadow-sm overflow-hidden">
      ...
      {/* Show more button - only when truncated and queryType is present (older payloads won't have it) */}
      {payload.isTruncated && payload.queryType && (
        <div className="flex flex-col items-center gap-1 py-2">
          <button
            onClick={handleShowMore}
            disabled={loadingMore}
            className="text-sm text-primary hover:text-primary/80 font-medium py-1.5 px-4 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {loadingMore ? (
