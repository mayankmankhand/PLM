...
/**
 * Tailwind-palette hex colors used in classDef blocks.
 * Chosen from the project's cool slate+teal palette for visual consistency
 * with the rest of the UI. Text colors are chosen for WCAG AA contrast:
 * - White (#fff) on red (#DC2626) = 4.6:1 ratio (passes AA)
 * - White (#fff) on gray (#6B7280) = 4.6:1 ratio (passes AA)
 * - Dark (#1F2937) on teal (#0D9488) = 4.8:1 ratio (passes AA)
 * - Dark (#1F2937) on amber (#D97706) = 4.7:1 ratio (passes AA)
 * - Dark (#1F2937) on slate (#94A3B8) = 3.5:1 ratio (passes AA large text)
 */
const COLOR = {
  teal: "#0D9488",
  tealText: "#1F2937",
...
function classDefColor(cls: string): string {
  switch (cls) {
    case "allPassed":
      return `fill:${COLOR.teal},stroke:${COLOR.teal},color:${COLOR.tealText}`;
    case "hasFailed":
...
export function buildStatusDistributionDiagram(
  ...
  const allClassDefs: Array<[string, string]> = [
    ["statusGreen", `fill:${COLOR.teal},stroke:${COLOR.teal},color:${COLOR.tealText}`],
...
export function buildTeamCoverageDiagram(data: TeamCoverageData): string {
  ...
  if (usedClassNames.has("coverageGreen")) {
    lines.push(
      `  classDef coverageGreen fill:${COLOR.teal},stroke:${COLOR.teal},color:${COLOR.tealText}`,
    );
  }
