...
// Simulate a PM user (Monica from Hardware team)
...
describe("Scenario 1: Happy path - full lifecycle journey", () => {
  ...
  it("Step 1: creates a product requirement in DRAFT", async () => {
    const pr = await prService.createProductRequirement(
      {
        ...
        description: "Battery must last 48 hours under normal usage",
      },
      ctx
    );
    prId = pr.id;
    ...
    expect(pr.title).toBe("Smartwatch Battery Life");
  });
    ...
    const sr = await srService.createSubRequirement(
      {
        ...
        description: "Verify battery consumption under continuous GPS tracking",
  ...
  it("Step 4: creates a test procedure (auto-creates draft v1)", async () => {
    const tp = await tpService.createTestProcedure(
      {
        title: "GPS Battery Drain Procedure",
        subRequirementId: srId,
        description: "Measures battery drain during 4-hour GPS tracking session",
        steps:
          "Step 1: Charge battery to 100%\n" +
          "Step 2: Enable continuous GPS tracking\n" +
          "Step 3: Run for 4 hours\n" +
