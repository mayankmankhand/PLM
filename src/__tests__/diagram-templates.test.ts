          ...
          testProcedures: [
            {
              ...
              totalTestCases: 3,
              testCases: [
                { id: "TC-1", title: "Resting HR", status: "PASSED" },
                { id: "TC-2", title: "Active HR", status: "PASSED" },
                { id: "TC-3", title: "Sleep HR", status: "FAILED" },
              ],
            },
          ],
        },
...
describe("buildTraceabilityDiagram", () => {
  ...
  it("includes PR node with correct label", () => {
    const result = buildTraceabilityDiagram(TRACEABILITY_FIXTURE, "summary");
    expect(result).toContain('pr_PR001["PR-001 Health Monitoring"]');
  });

  it("includes SR node with team name", () => {
    const result = buildTraceabilityDiagram(TRACEABILITY_FIXTURE, "summary");
    expect(result).toContain('sr_SR1["SR-1 Heart Rate Algorithm"]');
  });

  it("includes TP node", () => {
    const result = buildTraceabilityDiagram(TRACEABILITY_FIXTURE, "summary");
    expect(result).toContain('tp_TP1["TP-1 Algo Accuracy"]');
  });
  ...
  it("includes individual TC nodes in detailed mode", () => {
    ...
    expect(result).toContain("TC-1 Resting HR");
    expect(result).toContain("TC-2 Active HR");
    expect(result).toContain("TC-3 Sleep HR");
...
describe("buildTeamCoverageDiagram", () => {
  ...
  it("sorts teams alphabetically", () => {
    const result = buildTeamCoverageDiagram(COVERAGE_FIXTURE);
    const algoPos = result.indexOf("Algorithm");
    const appPos = result.indexOf("App");
    expect(algoPos).toBeLessThan(appPos);
  });
